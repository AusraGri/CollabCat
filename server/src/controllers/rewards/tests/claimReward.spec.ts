...
const mockRepo = (data: {reward?: Partial<PublicReward>, points?: Partial<PointsPublic>, claimed?: boolean}) => ({
  pointsRepository: {
    getPoints: vi.fn(async () => data.points as PointsPublic || undefined),
  } satisfies Partial<PointsRepository>,
  rewardsRepository: {
    getRewards: vi.fn(async()=> (data.reward ? [data.reward] : []) as PublicReward[]),
...
it('should throw error if input is not valid', async () => {
  ...
  // @ts-expect-error
  await expect(claimReward(inputTwo)).rejects.toThrowError(
    /invalid_type/i
  )
  // @ts-expect-error
  await expect(claimReward(inputThree)).rejects.toThrowError(/unrecognized_keys/i)
  // @ts-expect-error
