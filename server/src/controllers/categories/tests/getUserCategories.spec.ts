...
const categoryRepos = (category?: Partial<CategoriesPublic>) => ({
  categoriesRepository: {
    getPersonalCategoriesByUserId: vi.fn(async () =>  category ? [(category as CategoriesPublic)] : []),
...
it('find group categories by group id', async () => {
 ...
 const groupCategories = await getUserCategories()
  ...
  expect(repo.categoriesRepository.getPersonalCategoriesByUserId).toBeCalledWith(
  user.id
  )
})
