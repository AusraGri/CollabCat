...
export async function down(db: Kysely<any>) {
  await db.schema
  .dropIndex('idx_user_auth0_id')
  .execute()

await db.schema
  .dropIndex('idx_user_email')
  .execute()

await db.schema
  .dropIndex('idx_groups_created_by_user_id')
  .execute()

await db.schema
  .dropIndex('idx_user_groups_user_id')
  .execute()

await db.schema
  .dropIndex('idx_user_groups_group_id')
  .execute()

await db.schema
  .dropIndex('idx_categories_group_id')
  .execute()

await db.schema
  .dropIndex('idx_categories_created_by_user_id')
  .execute()

await db.schema
  .dropIndex('idx_recurring_pattern_task_id')
  .execute()

await db.schema
  .dropIndex('idx_completed_tasks_task_id')
  .execute()

await db.schema
  .dropIndex('idx_completed_tasks_instance_date')
  .execute()

await db.schema
  .dropIndex('idx_points_user_id')
  .execute()

await db.schema
  .dropIndex('idx_points_group_id')
  .execute()

await db.schema
  .dropIndex('idx_rewards_group_id')
  .execute()

await db.schema
  .dropIndex('idx_rewards_created_by_user_id')
  .execute()

await db.schema
  .dropIndex('idx_point_claims_user_id')
  .execute()

await db.schema
  .dropIndex('idx_point_claims_task_id')
  .execute()
}
